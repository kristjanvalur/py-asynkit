...
typedef struct {
    PyObject_HEAD PyObject *corostart; /* Reference to our CoroStart object */
...
typedef struct {
    PyObject_HEAD PyObject *wrapped_coro;
    PyObject *context;
...
static PyObject *corostart_wrapper_send(CoroStartWrapperObject *self, PyObject *arg)
{
    ...
    if(corostart->s_value != NULL || corostart->s_exc_type != NULL) {
        ...
        if(corostart->s_exc_type != NULL) {
            ...
            /* Clear the fields (PyErr_Restore steals references) */
        ...
        if(corostart->s_value != NULL) {
            /* Coroutine yielded a value - return it */
            PyObject *result = corostart->s_value;
            corostart->s_value = NULL; /* Clear and transfer ownership */
            return result;
        }
...
static PyObject *corostart_throw(CoroStartObject *self,
                                 PyObject *args,
                                 PyObject *kwargs)
{
    PyObject *exc;
    ...
    for(long i = 0; i < tries; i++) {
        ...
        if(self->context != NULL) {
            ...
            PyObject *throw_method = PyObject_GetAttrString(self->wrapped_coro,
                                                            "throw");
            if(throw_method == NULL) {
                Py_DECREF(value);
                return NULL;
            ...
            result =
                PyObject_CallMethod(self->context, "run", "OO", throw_method, value);
            Py_DECREF(throw_method);
        } else {
        ...
        if(PyErr_ExceptionMatches(PyExc_StopIteration)) {
            ...
            if(stop_value == NULL) {
                // If we couldn't get the value attribute, use the exception value
                // itself
    ...
    PyObject *exc_type_name = PyObject_GetAttrString((PyObject *) Py_TYPE(value),
                                                     "__name__");
...
static PyObject *corostart_close(CoroStartObject *self, PyObject *args)
{
    ...
    if(self->context != NULL) {
        ...
        PyObject *result =
            PyObject_CallMethod(self->context, "run", "O", close_callable);
...
static PyMethodDef corostart_methods[] =
    {{"done",
      ...
      METH_NOARGS,
      "Return True if coroutine is done"},
     {"result",
