...
per-drive session id for the duration of the pump. Active sessions are tracked on
a per-thread stack; contexts copied into `asyncio` tasks keep a stale session id
after the drive ends. Use `drive_async()` rather than `coro_drive()` alone when
