...
async def test_simple_case():
    ...
    print(f"s_value is None: {cs.s_value is None if hasattr(cs, 's_value') else 'no s_value attr'}")
    print(f"s_exc_type is None: {cs.s_exc_type is None if hasattr(cs, 's_exc_type') else 'no s_exc_type attr'}")
