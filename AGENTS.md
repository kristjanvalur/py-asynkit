# Agent Instructions for py-asynkit

Instructions for coding agents (Cursor, GitHub Copilot, Codex, and others) working in this repository.
...
- Optional C extension (`_cext`) for performance-critical primitives; `get_implementation_info()` reports active implementation
...
py-asynkit/
├── src/asynkit/
│   ├── coroutine.py       # Core coroutine tools (CoroStart, eager, coro_drive, await_sync)
...
│   ├── tools.py           # Helper utilities (cancelling context, create_task alias)
│   ├── compat.py          # asyncio version compatibility (eager tasks, PyTask patches)
│   ├── _cext/             # C extension sources (CoroStart, coro_drive optimizations)
│   ├── loop/              # Custom event loop implementations
│   │   ├── eventloop.py   # Extended event loop base and policies
│   │   ├── schedulingloop.py  # AbstractSchedulingLoop
│   │   ├── default.py     # Scheduling loop helpers (queue manipulation)
...
│   └── typing/            # Type-check tests (included in ty)
├── examples/              # Example code demonstrating features
├── scripts/               # C code formatting and build helpers
...
### C Extension Environment Variables

- `ASYNKIT_FORCE_CEXT=1`: Force C extension compilation; fail if it cannot be built (CI uses this)
- `ASYNKIT_DISABLE_CEXT=1`: Disable C extension, use pure Python
- `ASYNKIT_DEBUG=1`: Build C extension with debug symbols and assertions

### Common Tasks (using poethepoet)

- `uv run poe test` - Run tests (tests + examples)
- `uv run poe cov` - Run tests with coverage
- `uv run poe lint` - Run ruff linter
- `uv run poe format` - Format Python code with ruff
- `uv run poe format-c` - Format C extension sources
- `uv run poe format-docs` - Format markdown docs
- `uv run poe formatall` - Format Python, C, and docs
- `uv run poe typing` - Run ty type checking
- `uv run poe style` - Check Python formatting
- `uv run poe style-c` - Check C code formatting
- `uv run poe style-docs` - Check markdown formatting
- `uv run poe check` - Run all checks (style, style-c, style-docs, lint, typing, coverage)
...
- **Linting**: Ruff with E, F, I, UP rules enabled
- **Type Checking**: ty for `src/asynkit` and `tests/typing`, with targeted overrides for low-level operations
- **Docstrings**: Triple-quoted strings for public APIs
- **C Code**: Formatted via `scripts/format_c.py` (clang-format)
...
### Eager Task Factory - Global Eager Execution

For applications that want eager execution on all `asyncio.create_task()` calls:

```python
loop = asyncio.get_running_loop()
loop.set_task_factory(asynkit.eager_task_factory)

# Or with a custom Task subclass:
factory = asynkit.create_eager_task_factory(MyTask)
loop.set_task_factory(factory)
```

On Python < 3.12, use `asynkit.enable_eager_tasks()` from `compat.py` to monkeypatch asyncio with the same API.

### coro_drive() - Direct Coroutine Driving

`coro_drive()` drives a coroutine directly, invoking a callback for every yielded value. Uses the C extension when available.
...
`await_sync()`. Each drive gets a session id on a per-thread active stack; copied task
contexts keep a stale id after the pump ends. Custom pumps must use `drive_async()`,
not `coro_drive()` alone, when participating in this contract.
...
- `tests/typing/` contains type-check tests included in `ty`
...
### Eager Task Factory Testing

Many tests are parametrized to run with and without `asyncio.eager_task_factory` on Python 3.12+. See `tests/EAGER_TASK_TESTING.md` for details on which tests are compatible. Tests using the eager parametrization carry the `@pytest.mark.eager_tasks` marker.
...
- Minimum: Python 3.10
- CI tested on: 3.10, 3.11, 3.12, 3.13 (free-threaded), 3.14 (free-threaded), PyPy 3.11, GraalPy 3.12
...
- Wheels built for Windows, macOS, Linux (Python 3.10–3.14, including free-threaded)

### Backend Support

- **asyncio**: Full support (primary backend)
- **anyio**: Supported with asyncio backend
- **trio**: Limited support via anyio (trio 0.31.0+ supports Python 3.13+)

## Common Patterns to Follow

### Creating Tasks

`asynkit.tools.create_task` is an alias for `asyncio.create_task`. For eager-start behavior, use `eager_task_factory` or the `eager_start` parameter where supported.
...
- `DefaultSchedulingEventLoop` / `SchedulingSelectorEventLoop` - With task introspection
- `DefaultPriorityEventLoop` / `PrioritySelectorEventLoop` - With priority scheduling (experimental)
...
- Priority scheduling
- anyio eager task groups (`EagerTaskGroup`, `create_eager_task_group()`)

## Performance Considerations

- C extension provides ~4–5x speedup for `CoroStart` and `coro_drive` over pure Python
...
7. **Eager task factory incompatibility**: Many tests and patterns assume deferred task start; see `tests/EAGER_TASK_TESTING.md`
...
- Eager tasks comparison: docs/eager_tasks.md
- Eager task factory performance: docs/eager_task_factory_performance.md
